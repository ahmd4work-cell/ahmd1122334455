...
function listenToVisits() {
    ...
    onSnapshot(visitsRef, (snapshot) => {
        ...
        // حفظ العنصر النشط (Focus) لمنع فقدان التركيز أثناء التحديث اللحظي
        let activeId = null;
    ...
    mainRow.innerHTML = `
        ...
        <td class="col-status">
            <select class="excel-input status-select ${getStatusClass(v.status)}" onchange="handleStatusChange(this, '${rowId}')">
                <option value="جديدة" ${v.status === 'جديدة' || !v.status ? 'selected' : ''}>جديدة</option>
...
function addToActivityLog(actionField, oldVal, newVal, compName) {
    ...
    let message = '';
    if (actionField === 'إجراء' || actionField === 'الملاحظات') message = oldVal + (compName ? ` للشركة: ${compName}` : '');
    else if (oldVal && newVal) message = `تعديل ${actionField} لشركة (${compName}) من "${oldVal}" إلى "${newVal}"`;
    else if (!oldVal && newVal) message = `إضافة ${actionField} لشركة (${compName}): "${newVal}"`;
    else message = `تحديث ${actionField} لشركة (${compName})`;
...
async function insertNewRow() {
    ...
    const newVisit = {
        ...
        visitDate: getTodayDate(), curServ: '', oppValue: '0', notes: '[]',
        status: 'جديدة', editDate: getTodayDate(), owner: '', products: []
    };
    ...
    if (historyLog) {
        historyLog.innerHTML = arr.map(msg => {
            ...
            <div class="log-entry" style="display: block; line-height: 1.6;">
                ...
                <div class="log-action" style="padding-right: 5px; color: #0f172a; font-size: 11px; font-weight: 700;">${msg.text}</div>
            </div>
            `;
    ...
    if (action === 'حذف') {
        ...
        if (result.isConfirmed) {
            for (let cb of selectedRows) {
                ...
                if (tr && tr.id) await deleteDoc(doc(db, "visits", tr.id));
            }
...
// جعل الدوال متاحة عامةً للـ HTML
Object.assign(window, { insertNewRow, toggleSubTable, addProductRow, removeProductRow, calculateMainVisitValue, openNotesModal, closeNote, saveNote, handleStatusChange, toggleAllCheckboxes, toggleDropdown, handleBulkAction, toggleLogExpansion, debouncedFilterTable, updateWhatsAppLink, addToActivityLog });
