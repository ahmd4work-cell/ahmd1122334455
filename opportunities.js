// ==========================================
// 0. إعدادات واستيراد Firebase من الملف المركزي
// ==========================================
...
/* ==========================================================
   2. الاستماع اللحظي للفرص (لضمان استقبال المهلّلة من الزيارات فوراً)
   ========================================================== */
function listenToOpportunities() {
    ...
    onSnapshot(oppsRef, (snapshot) => {
        ...
        // منع التحديث الفجائي إذا كان المستخدم يكتب في حقل معين
        ...
        console.error("خطأ في استقبال الفرص البيعية من السحابة:", error);
    });
}

/* ==========================================================
   3. الدالة الأساسية لبناء السطور (renderRow) 
   ========================================================== */
function renderRow(v = {}, prepend = false) {
    ...
    // منع تكرار نفس الصف إذا كان موجوداً مسبقاً في الجدول
    ...
    // دعم الحقلين القادمين من الزيارات (oppDate أو visitDate)
    ...
    mainRow.innerHTML = `
        ...
        <td><div class="notes-preview" onclick="openNote(this)" data-full-notes='${notesJson.replace(/'/g, "&apos;")}' id="preview-${rowId}">${lastNoteText}</div></td>
        <td>
...
/* ==========================================================
   4. دوال جدول المنتجات داخل الفرصة
   ========================================================== */
...
/* ==========================================================
   5. الإجراءات الجماعية والبحث والفرز
   ========================================================== */
...
async function handleBulkAction(action) {
    ...
    if (action === 'حذف') {
        const result = await Swal.fire({ title: 'تأكيد الحذف؟', text: "سيتم حذف الفرص المحددة نهائياً من السحابة!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', cancelButtonColor: '#94a3b8', confirmButtonText: 'نعم، احذف', cancelButtonText: 'إلغاء' });
        if (result.isConfirmed) {
            for (const chk of selected) {
                const row = chk.closest('tr');
                const id = row.id;
                addToActivityLog('إجراء', 'حذف الفرصة البيعية', '', row.cells[1].querySelector('input').value);
                row.remove();
                if(document.getElementById('sub-' + id)) document.getElementById('sub-' + id).remove();
                try {
...
/* ==========================================================
   6. إدارة الملاحظات وتلميحات الحالة
   ========================================================== */
...
/* ==========================================================
   7. العمليات التشغيلية وتحديث التواريخ
   ========================================================== */
...
/* ==========================================================
   8. دمج الحالة ومتابعة الفرص وألوان التاريخ
   ========================================================== */
...
/* ==========================================================
   9. حفظ السحابة التلقائي والصامت
   ========================================================== */
...
/* ==========================================================
   10. منطق الـ Date Picker المنبثق والتهيئة
   ========================================================== */
...
function closeCustomDatePicker() {
    ...
    if(dpDropdown) dpDropdown.style.display = 'none';
    dpActiveInput = null;
    dpActiveRowId = null;
}
...
document.addEventListener('click', function(e) {
    ...
    if (dpDropdown && e.target !== dpMonthLabel && e.target !== dpYearLabel && !dpDropdown.contains(e.target)) {
        dpDropdown.style.display = "none";
    }
});

// تهيئة الصفحة عند التحميل بشكل متوافق مع Modules
document.addEventListener('DOMContentLoaded', initDatePicker);

// تصدير الدوال المطلوبة للاستخدام العام في الـ HTML
